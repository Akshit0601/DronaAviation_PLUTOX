...
class Pluto:
    ...
    def __init__(self,TCP_IP):
        ...
        # "{rcRoll: 1500, rcPitch: 1500, rcYaw: 1500, rcThrottle: 1000, rcAUX1: 1500, rcAUX2: 1500, rcAUX3: 1500, rcAUX4: 1500}"
    ...
    def takeoff(self):
        header = bytearray([36,77,60])
        
        valueArray=bytearray([])
        valueArray.extend(header)
        valueArray.append(2)      #length of payload
        valueArray.append(217)
        valueArray.extend([0,1])

        CRCArray=valueArray[3:-1]
        CRCValue=0
        for d in CRCArray:
            CRCValue= CRCValue^d
        valueArray.extend(CRCArray)
        self.mySocket.sendall(valueArray)
